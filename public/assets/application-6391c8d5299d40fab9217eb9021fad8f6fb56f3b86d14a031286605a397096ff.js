...
var railsAjax = {
    ...
    try {
      ...
      jQuery(window).bind('statechange', function() {
        try {
          ...
          alert('[RailsAjax] History statechange triggered #' + lState.data.id + ' via ' + lState.data.type + ' to ' + lState.url.replace(lRootURL, '/') + ' (anchor: ' + lState.data.anchor + ', dataType: ' + lState.data.dataType + ')');
  ...
  attachAjaxToElements: function(iCSSSelector) {
    try {
      alert('[RailsAjax] Attach Ajax bindings to elements ' + iCSSSelector + ' (' + jQuery(iCSSSelector).length + ' elements)');
      jQuery(document)
        .delegate(iCSSSelector, 'ajax:beforeSend', function(iEvent, ioXHR, iSettings) {
          try {
            ...
            alert('[RailsAjax] Ajax before send. URL=' + lURL + ' Method=' + iSettings.type + ' Data=' + iSettings.data);
            var lContinue = true;
            ...
            alert('[RailsAjax] Continue on the Ajax load ? ' + lContinue);
            if (lContinue) {
  ...
  ajaxSuccess: function(iXHR, iData) {
    try {
      alert('[RailsAjax] Ajax success. Loading data: ' + iData);
  ...
  pushNewState: function(iURL, iData, iReplaceState) {
    try {
      ...
      railsAjax.gStateID = railsAjax.gStateID + 1;
    } catch (iError) {
